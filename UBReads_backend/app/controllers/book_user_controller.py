...
class BookUserController:
    ...
    def insert_book_user(db: Session, user_id: int, book_id: int):
        stmt = book_users.insert().values(user_id=user_id, book_id=book_id, is_read=False)  # Por defecto, no leído
        db.execute(stmt)
        db.commit()
    ...
    def get_users_by_book(db: Session, book_id: int):
        return db.query(User).join(book_users).filter(book_users.c.book_id == book_id).all()

    @staticmethod
    def delete_book_user(db: Session, user_id: int, book_id: int):
        stmt = book_users.delete().where(book_users.c.user_id == user_id, book_users.c.book_id == book_id)
        db.execute(stmt)
        db.commit()
        
    @staticmethod
    def update_read_status(db: Session, user_id: int, book_id: int, is_read: bool):
        user_book_record = db.query(book_users).filter(
